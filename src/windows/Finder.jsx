...
const Finder = () => {
    ...
    const openItem = (item) => {
        ...
        // Will add working Links Shortly
        // if(["fig" , "url"].includes(item.fileType) && item.href)
        //     return window.open(item.href ,"_blank");
        console.log("inside finder" , item);
        openWindow(`${item.fileType}${item.kind}` , item);
    }


    const renderLists = (name , items) => (
        <div>
            <h3>{name}</h3>
            <ul>{
                items.map((item) => (
        ...
        <>
            ...
            <div className='bg-white flex h-full'>
                <div className='sidebar'>
                    {renderLists('Favourites' , Object.values(locations))}
                    {renderLists('My Projects' , locations.work.children)}
                </div>
